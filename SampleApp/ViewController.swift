    ...
    @IBAction func startSpeechRecognition(sender: Any) {
        ...
        recognitionTask = speechRecognizer.recognitionTask(with: recognitionRequest!) { result, error in
            if let result = result {
                ...
                self.speechInput = spokenText + " " // Aggregate spoken text
                self.resetSilenceTimer()
                
//                self.handleTimerCommand(spokenText)
//                self.displayTextOnGlasses(spokenText)
    ...
    private func resetSilenceTimer() {
        ...
        silenceTimer = Timer.scheduledTimer(withTimeInterval: 3.0, repeats: false) { [weak self] _ in
            ...
            self?.handleTimerCommand(self?.speechInput ?? "")
    ...
    private func parseTimerCommand(_ command: String) -> Int? { //TODO: change to LLM eventually
...
//        let components = command.components(separatedBy: CharacterSet(charactersIn: "0123456789"))
//        for component in components {
//            if let number = Int(component.trimmingCharacters(in: .whitespaces)), command.contains("minute") || command.contains("min") {
//                return number
//            }
//        }
//        return nil
